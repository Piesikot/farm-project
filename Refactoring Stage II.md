### Refactoring Stage II - migration typhogaphy and spacing units to rem/em.
...
- Fonts: Display = Playfair Display; Body = Inter (with fallbacks).
...
- Add a `:root` block at the top of style.css defining:
  - Fonts: `--font-display` (Playfair Display), `--font-body` (Inter), `--font-fallback: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif`.
  ...
  - Colors: `--color-text, --color-bg, --color-text-inverse, --color-bg-inverse`.
- Inventory shared image sizes and spacing values in style.css (logo, advantages icons, polish seal, worldwide globe, header/menu paddings) and convert them into tokens before replacing declarations.
- Introduce image size tokens (e.g., `--size-logo`, `--size-advantage-icon`, `--size-seal`, `--size-globe`) and tokenized padding/gap values for shared components; keep the 1050px breakpoint values tokenized.
- Comment out old declarations and add the tokenized version immediately after, to avoid ambiguity and keep rollback simple.

## Execution order
1) Inventory current shared image sizes and spacing values in style.css (shared components only).
2) Add :root tokens for fonts, typography (clamp-based), spacing, colors, and shared image sizes.
3) Apply global font-family (Playfair Display for headings if scoped, Inter for body) and update selectors to use tokens.
